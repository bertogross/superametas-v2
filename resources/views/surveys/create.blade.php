                ...
                <div class="card-header">
                    <div class="float-end">
                        @if ($survey)
                            ...
                            <a href="{{ route('surveysComposeShowURL', ['id' => $survey->id]) }}" class="btn btn-sm btn-outline-dark" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Visualizar em nova guia" target="_blank" tabindex="-1">Visualizar</a>
                        @else
                            <button type="button" class="btn btn-sm btn-theme" id="btn-surveys-store-or-update" tabindex="-1">Salvar</button>
                        @endif
                    </div>
                    <h4 class="card-title mb-0"><i class="ri-drag-drop-line fs-16 align-middle text-theme me-2"></i>Formulário de Vistoria</h4>
                </div>
                <div class="card-body">
                    <div class="row">
                            ...
                            <div class="p-3 border border-1 border-light rounded">
                                <form id="surveysForm" method="POST" autocomplete="off" class="needs-validation" novalidate autocomplete="false">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <div class="form-text">Selecione a loja que será vistoriada</div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <textarea name="description" class="form-control" maxlength="1000" id="description" rows="8" maxlength="500">{{ $description }}</textarea>
                        ...
                        <div class="col-sm-6 col-md-7 col-lg-8 col-xxl-9">
                            <div class="p-3 border border-1 border-light rounded">
                                <p class="text-muted mb-3">Selecione o formulário que será anexado a esta vistoria e complemente delegando a cada dos blocos o respectivo usuário responsável pela etapa/setor/departamento.</p>
                                ...
                                <div class="tab-content text-muted p-3 border border-1 border-light rounded rounded-top-0">
                                    <div class="tab-pane active" id="arrow-departments" role="tabpanel">
                                        <h6>Departamentos</h6>
                                        <div class="mb-3">
                                            <select class="form-select select-survey-form" name="survey_compose_default_id" data-id="default" data-selected-value="{{ old('survey_compose_default_id', $surveyComposeDefaultId ?? '') }}" autocomplete="off">
                                                <option value="">- Selecionar Formulário -</option>
                                                @foreach ($getSurveyComposeDefault as $default)
                                                    <option value="{{ $default->id }}" {{ (old('survey_compose_default_id', $surveyComposeDefaultId ?? '') == $default->id) ? 'selected' : '' }}>
                                                        {{ $default->title }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div id="load-preview-default" class="border border-1 rounded border-light p-3">O formulário selecionado será exibido aqui</div>
                                    </div>
                                    <div class="tab-pane" id="arrow-custom" role="tabpanel">
                                        <h6>Formulário Customizado</h6>
                                        <div class="mb-3">
                                            <select class="form-select select-survey-form" name="survey_compose_custom_id" data-selected-value="{{ old('survey_compose_custom_id', $surveyComposeCustomId ?? '') }}" data-id="custom" autocomplete="off">
                                                <option value="">- Selecionar Formulário -</option>
                                                @foreach ($getSurveyComposeCustom as $custom)
                                                    <option value="{{ $custom->id }}" {{ (old('survey_compose_custom_id', $surveyComposeCustomId ?? '') == $custom->id) ? 'selected' : '' }}>
                                                        {{ $custom->title }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div id="load-preview-custom" class="border border-1 rounded border-light p-3">O formulário selecionado será exibido aqui</div>
                                    </div>
                                </div>
                            </div>
                        </div>
