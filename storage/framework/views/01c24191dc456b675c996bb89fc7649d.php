        ...
        <div class="row mb-3">
            <div class="col-md-12 col-lg-6 col-xxl-7">
                <div class="card h-100">
                    <form id="auditsComposeForm" method="POST" autocomplete="off" class="needs-validation" novalidate autocomplete="false">
                        <?php echo csrf_field(); ?>
                        ...
                        <div class="card-header">
                            <div class="float-end">
                                <button type="button" class="btn btn-sm btn-theme" id="btn-audits-compose-store-or-update" tabindex="-1">
                                    <?php if($data): ?>
                                        Atualizar
                                    <?php else: ?>
                                        Salvar
                        ...
                        <div id="nested-compose-area" class="card-body pb-0" style="min-height: 250px;">
                                    ...
                                    <?php if($data): ?>
                                        <span class="btn-group">
                                            <?php if($data->status == 'active'): ?>
                                                <button type="button" class="btn btn-sm btn-outline-danger" id="btn-audits-compose-toggle-status" data-status-to="disabled" data-compose-id="<?php echo e($data->id); ?>" tabindex="-1">Desativar</button>
                                            <?php else: ?>
                                                <button type="button" class="btn btn-sm btn-outline-success" id="btn-audits-compose-toggle-status" data-status-to="active" data-compose-id="<?php echo e($data->id); ?>" tabindex="-1">Ativar</a>
                                            <?php endif; ?>
                            ...
                            <?php if( !$topicsData && $type == 'default' ): ?>
                                <?php
                                    $defaultTopics = file_get_contents(resource_path('views/audits/demo/default-audit-topics.json'));
                                ...
                                <?php $__currentLoopData = $topicsData; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $stepIndex => $step): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    ...
                                    <div id="<?php echo e($originalIndex); ?>" class="accordion-item block-item mt-0 mb-3 border-dark p-0">
                                        ...
                                        <div class="accordion-collapse collapse show">
                                            <?php echo $__env->make('audits.includes.topics-input', [
                                                'type' => $type,
            ...
            <div class="col-md-12 col-lg-6 col-xxl-5">
                <div class="card h-100">
                    <div class="card-header">
                        <div class="float-end">
                            <?php if($data): ?>
                                <a href="<?php echo e(route('auditsComposeShowURL', ['id' => is_array($data) ? $data['id'] : $data->id])); ?>" class="btn btn-sm btn-outline-theme position-absolute me-3 end-0" data-bs-toggle="tooltip" data-bs-trigger="hover" data-bs-placement="top" title="Visualizar em nova guia" target="_blank" tabindex="-1"><i class="ri-eye-line"></i></a>
                            <?php endif; ?>
                        </div>
...
<?php $__env->startSection('script'); ?>
    ...
    <script src="<?php echo e(URL::asset('build/js/audits-sortable.js')); ?>" type="module"></script>

    <script>
        var auditsComposeShowURL = "<?php echo e(route('auditsComposeShowURL')); ?>";
        var auditsComposeCreateOrUpdateURL = "<?php echo e(route('auditsComposeCreateOrUpdateURL')); ?>";
